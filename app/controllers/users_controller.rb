class UsersController < ApplicationController
  ...
  def create
    ...
    if @user.save
      ...
      flash[:notice] = "Welcome to the BeVokal Blog, #{@user.username}"
      redirect_to articles_path
    else
      render "new"
    end
  ...
  def search
    if params[:query].present?
      user = User.find_by("username LIKE ? OR email LIKE ?",
                       "%#{params[:query]}%",
                       "%#{params[:query]}%")
        if user
          redirect_to user_path(user) and return
        end
      article = Article.find_by("title LIKE ? OR description LIKE ?",
                             "%#{params[:query]}%",
                             "%#{params[:query]}%")
